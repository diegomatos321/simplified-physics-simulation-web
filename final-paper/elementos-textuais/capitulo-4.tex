...
Para aplicações que exigem resposta física também é necessário computar o deslocamento mínimo para separação dos objetos. Seja $\vec{d}$ o \textit{Minimum Translation Vector} (MTV), um vetor que representa a menor correção necessária para eliminar a interpenetração entre os objetos orientado do corpo $A$ em direção ao corpo $B$.
...
O GJK constrói iterativamente um \textit{simplex} (ponto, segmento, triângulo ou tetraedro, dependendo da dimensão) que aproxima a origem dentro de $A - B$.  
