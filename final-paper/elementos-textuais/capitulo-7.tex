...
A forma mais ingênua de simular física é utilizar o tempo decorrido entre quadros (\emph{delta time}) como passo de simulação. Embora simples, isso introduz instabilidade numérica: simulações podem divergir em altas taxas de quadros, apresentar \emph{tunneling} e se comportar de maneira não determinística.
...
Em programas interativos tradicionais, a lógica, a física e a renderização são executadas em um único \emph{thread}. Nesse modelo, a renderização só ocorre após a conclusão da etapa de física, e qualquer uma das etapas pode se tornar gargalo.
