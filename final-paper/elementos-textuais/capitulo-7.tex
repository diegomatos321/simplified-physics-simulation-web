...
A eficiência computacional é um dos fatores determinantes para o desempenho de um motor de 
física em tempo real. Em jogos, animações interativas, simulações físicas e aplicações 
gráficas, a demanda por atualizações contínuas e a necessidade de operar dentro de limites 
rigorosos de tempo tornam indispensável o uso de técnicas de otimização em todos os estágios 
...
Neste capítulo descrevemos as principais estratégias empregadas 
na otimização da detecção e resolução de colisões, com foco na divisão entre \emph{Broad Phase} 
e \emph{Narrow Phase}, no uso de estruturas espaciais, na adoção de passos de simulação fixos 
e em técnicas de paralelização por múltiplos núcleos de processamento.

\section{Broad Phase}

A fase de \emph{Broad Phase} tem como objetivo eliminar pares de objetos que certamente 
não estão colidindo, reduzindo o conjunto de pares candidatos antes da execução de testes 
mais complexos.  
...
\begin{algorithm}
	...
	\KwOut{Pares de objetos passíveis a colisão}
	vistos $\leftarrow$ \{\} \\
...
Após a Broad Phase reduzir a lista de pares, a \emph{Narrow Phase} realiza testes 
geométricos precisos. Esta fase utiliza algoritmos complexos como:
...
\begin{algorithm}
	...
	tempo anterior $\leftarrow$ agora() \\
	\While{!sair}{
...
\begin{itemize}
	...
	\item previsibilidade temporal,
	\item resultados reprodutíveis,
	\item funcionamento adequado de restrições e colisões.
\end{itemize}
