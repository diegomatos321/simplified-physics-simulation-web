...
Nos métodos \textit{Metodo de Penalidades}, colisões são tratadas como interpenetrações que geram forças de repulsão proporcionais à profundidade de penetração alterando diretamente a acelaração. Essas forças geralmente seguem modelos de mola e amortecimento, trata-se de um método simples, porém sensível à escolha dos parâmetros de rigidez, podendo causar instabilidade numérica.
...
Após a etapa de detecção de colisão, seja $\vec{d}$ o \textit{Minimum Translation Vector} (MTV), um vetor que representa a menor correção necessária para eliminar a interpenetração entre os objetos. O MTV é orientado do corpo $A$ em direção ao corpo $B$.
...
Como discutido anteriormente, o objetivo principal dessas técnicas não é fidelidade física absoluta, mas sim a geração de comportamentos verossímeis, estáveis e visualmente plausíveis. No entanto, essa busca por simplicidade e desempenho resulta em compromissos importantes.
