...
A abordagem mais comum para simular deformações em jogos e aplicações interativas é o modelo \textbf{Massa-Mola} (\textit{Mass-Spring System}). Neste modelo, um objeto é discretizado em um conjunto de partículas pontuais com massa $m$, conectadas por molas ideais sem massa.
...
\begin{equation}
	\vec{F}_{amortecimento} = -k_d (\vec{v}_i - \vec{v}_j) \cdot \frac{\vec{x}_i - \vec{x}_j}{|\vec{x}_i - \vec{x}_j|},
\end{equation}
...
Para simular superfícies deformáveis, como tecidos, as partículas são organizadas em uma malha (\textit{grid}). A estabilidade e o comportamento visual do tecido dependem de como essas molas (ou restrições) são conectadas. Uma topologia robusta geralmente emprega três tipos de conexões:

\begin{enumerate}
	\item \textbf{Molas Estruturais (\textit{Structural}):} Conectam partículas vizinhas diretas (horizontal e verticalmente). Elas resistem à tração e compressão básicas, mantendo a integridade da malha.
	\item \textbf{Molas de Cisalhamento (\textit{Shear}):} Conectam partículas diagonalmente vizinhas. Sua função é impedir que o tecido se distorça ou "deslize" sobre si mesmo, mantendo a estabilidade dos quadriláteros da malha.
	\item \textbf{Molas de Flexão (\textit{Bend}):} Conectam partículas alternadas (pulando um vizinho). Elas resistem à dobra do tecido, impedindo que ele se comporte como uma malha infinitamente flexível e conferindo-lhe uma certa rigidez à curvatura.
\end{enumerate}
